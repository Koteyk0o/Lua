...
if component.isAvailable('capacitor_bank') then
	eio_capacitor = component.capacitor_bank
end
...
local ME_Filter = {
	name = 'BigReactors:BRIngot' -- ID слитка йеллоурита
}
...
function data_Collector()
	if component.isAvailable('br_reactor') then
		...
		raw_Data['reactor_Fuel_Info']['fuelAmount'] = reactor.getFuelAmount()
		raw_Data['reactor_Fuel_Info']['fuelCapacity'] = reactor.getFuelAmountMax()
		raw_Data['reactor_Fuel_Info']['fuelConsumedLastTick'] = reactor.getFuelConsumedLastTick()
		raw_Data['reactor_Energy_Info']['energyProducedLastTick'] = reactor.getEnergyProducedLastTick()
		raw_Data['reactor_Energy_Info']['energyStored'] = reactor.getEnergyStored()
